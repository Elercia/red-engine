...
constexpr inline Vector4 operator*(const Vector4& v, const Matrix44& m)
{
    Vector4 ret;

    ret.x = m(0, 0) * v.x + m(1, 0) * v.y + m(2, 0) * v.z + m(3, 0) * v.w;
    ret.y = m(1, 0) * v.x + m(1, 1) * v.y + m(2, 1) * v.z + m(3, 1) * v.w;
    ret.z = m(2, 0) * v.x + m(1, 2) * v.y + m(2, 2) * v.z + m(3, 2) * v.w;
    ret.w = m(3, 0) * v.x + m(1, 3) * v.y + m(2, 3) * v.z + m(3, 3) * v.w;
