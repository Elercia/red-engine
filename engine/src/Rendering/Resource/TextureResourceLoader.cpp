...
bool TextureResourceLoader::InitResource(std::shared_ptr<Texture2D>& resource, const Path& path, nlohmann::json jsonContent)
{
    std::string asciiPath = jsonContent["path"];

    int nrChannels;
    unsigned char* data =
        stbi_load(asciiPath.c_str(), &resource->m_size.width, &resource->m_size.height, &nrChannels, 0);

    if (data == nullptr)
    {
        RED_LOG_WARNING("Error creating surface from texture path {} with error {}", path.GetAscciiPath(), stbi_failure_reason());
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, resource->m_size.width, resource->m_size.height, 0, GL_RGB,
                 GL_UNSIGNED_BYTE,
